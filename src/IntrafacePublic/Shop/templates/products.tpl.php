    ...
    <tr>
        <td class="picture">
            <?php if (array_key_exists(0, $product['pictures'])): ?>
                <a href="<?php e($this->urlToProductId($product['id'])); ?>"><img src="<?php e($product['pictures'][0]['thumbnail']['file_uri']); ?>" alt="<?php e($product['name']); ?>" height="<?php e($product['pictures'][0]['thumbnail']['height']); ?>" width="<?php e($product['pictures'][0]['thumbnail']['width']); ?>" /></a>
            <?php endif; ?>
        </td>
        <td class="name"><a href="<?php e($this->urlToProductId($product['id'])); ?>"><?php e($product['name']); ?></a></td>
        <td class="price" nowrap="nowrap"><?php e($currency); ?> <?php $price = new Ilib_Variable_Float($product['currency'][$currency]['price_incl_vat']); e($price->getAsLocal($this->document->locale, 2)); ?></td>
        <td  nowrap="nowrap" class="buy">
            <?php if ($product['has_variation']): ?>
                <a class="details" href="<?php e($this->urlToProductId($product['id'])); ?>"><?php e(__('Details')); ?></a>
